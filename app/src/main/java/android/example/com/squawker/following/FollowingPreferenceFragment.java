...
public class FollowingPreferenceFragment extends PreferenceFragmentCompat implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    // TODO (2) When a SharedPreference changes, check which preference it is and subscribe or un-subscribe to the correct topics.
    // Ex. FirebaseMessaging.getInstance().subscribeToTopic("key_lyla"); subscribes to Lyla's squawks.
    // HINT: Checkout res->xml->following_squawker.xml. Note how the keys for each of the preferences matches the topic to subscribe to for each instructor.
    /**
